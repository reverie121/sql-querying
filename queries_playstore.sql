...
SELECT category, COUNT(*) AS qty FROM analytics
GROUP BY category;
...
SELECT category, ROUND(AVG(CAST(rating AS NUMERIC)), 3) as avg_rating
FROM analytics
GROUP BY category
...
-- 13. Count all the reviews in the Google Play Store. For a total number of reviews: --
SELECT SUM(reviews)
FROM analytics;
-- For the number of apps with reviews: --
